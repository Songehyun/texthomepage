...
function chooseAdventure(choice) {
  ...
  if (choice === 1) {
    storyText =
      '텍스트 어드벤처로 만들어진 홈페이지라니,<br>당신은 처음보는 홈페이지에 크게 놀랐다! <br>적어도 개발자는 만들면서 놀라고 있다!<br>What a Awesome idea!';
  } else if (choice === 2) {
    storyText =
      '개발자의 이름, 개발자의 나이, 개발자가 좋아하는것... <br>통찰력이 뛰어난 당신은 단번에 이 홈페이지가<br>이 개발자를 소개하는 홈페이지라는 것을 파악했다.';
  } else if (choice === 3) {
    storyText =
      '아무래도 개발자는 예전에 폴더폰을 이용해<br>텍스트 어드벤처를 즐겨 했던 모양이다. <br>당신은 폴더폰이 뭔지 검색해보기로 했다.';
  } else if (choice === 4) {
    storyText =
      '당신은 개발자라고 쓰여진 방을 찾았다. <br>방문을 열고 들어가니<br>홈페이지 개발자에 대한 다양한 정보들이 있었다.';
